...
//
void Tokenizer::SplitString(const string & arg, vector<string>& vTokens, int line, bool& labelExists)
{
	string curStr = "";
	...
	for (; idx < arg.size(); idx++) {
		if (DELIMITERS.find(string(1, arg[idx])) == DELIMITERS.end()) {			
			if ((idx > 0) && (arg[idx - 1] == ' ') && !pushed && (curStr != "") ) {
				if (space)
					throw WrongInstructionFormat(line, " spaces");
				if (curStr == ".long") {
					curInstr = curStr;
					DELIMITERS.insert("+");
					DELIMITERS.insert("-");
					if ((arg[idx] == '+') || (arg[idx] == '-')) {
						// Sledeci put smatrace da je validan znak
						//
						idx--;
						space = true;
						continue;
			...
			if ((arg[idx] == ':')) {
				...
				else throw WrongInstructionFormat(line, " label ");
			}
			else if ((arg[idx] == ',')) {
				if (((vTokens.size() - (int)labelExists) > 0) && curStr != "") {
					...
					throw WrongInstructionFormat(line, " comma");
			}
			else if ( (arg[idx] == '+') || (arg[idx] == '-'))  {
					if ((curStr != "") || lastTokenComma){
					...
					else throw WrongInstructionFormat(line, " + or -");
				}
	...
	if (!lastTokenValid)
		throw WrongInstructionFormat(line, "last token");
	if (!pushed && curStr != "")
		vTokens.push_back(curStr);
	/*
...
int Tokenizer::ConvertImmediateNumber(const string& arg) {
	int idx = 0;
	string copyArg = arg;
	ToLower(copyArg);
	int val = 0;
...
bool Tokenizer::isChar(const string & arg)
{
	if (arg.length() != 3)
		return false;
	if ((arg[0] != '\'') && (arg[2] != '\'')) {
...
int Tokenizer::ConvertChar(const string & arg)
{
	return arg[1];
}
