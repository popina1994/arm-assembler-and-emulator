...
public:
	...
	bool operator==(const SymbolTableEntry&) const;
	SymbolTableEntry(string _name, string _visibility, int _value, string _typeOfSection ="", int _size = -1) : name(_name), visibility(_visibility), value(_value),
		size(_size), typeOfSection(_typeOfSection), id(globalID++), section(NULL) {}
