...
public:
	...
	//
	void SplitString(const string & arg, vector<string>& vTokens, int line, const string& loaderName);
	...
	int ConvertImmediateNumber(const string& arg, bool hex = false);
	// Check is directive.
	//
	bool isDirective(const string& arg, Wraper&);

	void SplitStringSpace(const string & arg, vector<string>& vTokens, bool special = false);
